...
const CAPABILITY_TAGS = {
  Healthcare: ["24/7-capable", "Human handoff", "CRM-ready"],
  EdTech: ["Lead capture", "Follow-up", "Fee reminders"],
  Insurance: ["Renewal flows", "Document intake", "Escalation"],
  "E-commerce": ["Support", "Cart follow-up", "Order workflows"],
  HR: ["Screening assist", "Scheduling", "Onboarding flows"],
};
  ...
  return (
    <Section id="industries" eyebrow="INDUSTRY SOLUTIONS">
      <h2 className="syne" style={{ fontSize: "clamp(28px,5vw,58px)", fontWeight: 800, lineHeight: 1.08, marginBottom: 52 }}>
        ...
        <span style={{ color: G.green }}>One operating principle.</span>
      </h2>
      ...
      <div className="grid-split" style={{ gap: 48, alignItems: "start" }}>
        <div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 36 }}>
            {CAPABILITY_TAGS[activeNiche].map((tag) => (
              <span
                key={tag}
          ...
          >
            <span style={{ color: "inherit" }}>[ Explore {activeNiche} Demo → ]</span>
          </button>
        </div>
        ...
        <div className="grid-cards-2" style={{ gap: 12 }}>
          {nicheData.features.map((feat) => (
            <div key={feat} style={{ background: G.card, border: `1px solid ${G.border}`, padding: "20px 18px", position: "relative", overflow: "hidden" }}>
              ...
              <div className="mono" style={{ color: G.muted, fontSize: 10, lineHeight: 1.7 }}>Configured around client workflow and guardrails.</div>
