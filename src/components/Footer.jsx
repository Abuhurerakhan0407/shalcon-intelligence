...
/**
 * Footer — 4-column links (brief §4 footer, §3 real contact links).
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div className="grid-cards-4" style={{ gap: 48, marginBottom: 56 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
              ...
              <div>
                <div className="syne" style={{ color: G.green, fontSize: 13, fontWeight: 800, letterSpacing: ".25em", lineHeight: 1 }}>
                  {SITE_CONFIG.agencyName}
                </div>
                <div className="mono" style={{ color: G.muted, fontSize: 8, letterSpacing: ".18em" }}>
                  {SITE_CONFIG.tagline}
                </div>
              </div>
            </div>
            <p className="mono" style={{ color: G.muted, fontSize: 12, lineHeight: 1.9, maxWidth: 250, marginBottom: 24 }}>
          ...
          <div>
            ...
            {Object.keys(NICHES).map((l) => (
              <button key={l} className="footer-link" onClick={() => scrollTo("industries")}>{l}</button>
            ))}
          </div>
          ...
          <div>
            ...
            {SERVICES.map((s) => (
              <button key={s.title} className="footer-link" onClick={() => scrollTo("services")}>{s.title}</button>
            ))}
          </div>
          ...
          <div>
            ...
            {[
              ...
              <button key={item.label} className="footer-link" onClick={item.action}>{item.label}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ borderTop: `1px solid ${G.border}`, paddingTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <span className="mono" style={{ color: "#333355", fontSize: 11 }}>
            <span style={{ color: G.green }}>›</span> Systems online. Automations running. Ready when you are._
          </span>
