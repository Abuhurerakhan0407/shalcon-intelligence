...
export default function DemoWidget() {
  ...
  const sendChat = () => {
    ...
    setIsTyping(true);
    setTimeout(() => {
      const responses = DEMO[demoNiche].responses;
      setChatMsgs((current) => [...current, { r: "bot", t: responses[Math.floor(Math.random() * responses.length)] }]);
      setIsTyping(false);
    }, 700);
  };
      ...
      <div className="grid-split" style={{ gap: 64, alignItems: "start" }}>
        ...
        <div>
          <div style={{ background: "#04040E", border: `1px solid ${G.border}`, overflow: "hidden", boxShadow: "0 0 80px rgba(0,255,148,.05)" }}>
            ...
            <div role="log" aria-live="polite" aria-label={`${demoNiche} prototype conversation`} style={{ height: 320, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10, background: "linear-gradient(180deg,#060610,#030308)" }}>
