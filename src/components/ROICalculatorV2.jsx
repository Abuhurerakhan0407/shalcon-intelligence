  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await submitLead({
        ...
        avgTxn: avgValue,
        estimatedDailyLoss: Math.round(estimate.daily),
        estimatedMonthlyLoss: Math.round(estimate.monthly),
        estimatedYearlyLoss: Math.round(estimate.yearly),
        conversionRate,
      });
      setState("confirmed");
    ...
    return (
      <div style={{ minHeight: "70vh", display: "grid", placeItems: "center", padding: 28, color: G.white }}>
        <div style={{ maxWidth: 620, textAlign: "center", background: G.card, border: `1px solid ${G.green}44`, padding: "44px 28px" }}>
          ...
          <p className="mono" style={{ color: G.muted, lineHeight: 1.8, marginBottom: 28 }}>
            We saved the details you submitted. You can also choose a call slot now or message Shalcon directly.
          </p>
    ...
    <div style={{ background: G.bg, color: G.white, minHeight: "100vh", padding: "clamp(28px,6vw,70px) clamp(16px,4vw,36px)" }}>
      <div style={{ maxWidth: 1040, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 42 }}>
          ...
          <p className="mono" style={{ maxWidth: 720, margin: "0 auto", color: G.muted, lineHeight: 1.8, fontSize: 12 }}>
            This is a planning estimate, not a promise of lost or recoverable revenue. Adjust the assumptions to match your own data before using the result in a business decision.
          </p>
        </div>
        ...
        <div className="grid-split" style={{ gap: 18, alignItems: "start" }}>
          <div style={{ background: G.card, border: `1px solid ${G.border}`, padding: "26px 22px" }}>
            <label className="mono" style={{ display: "block", color: G.muted, fontSize: 10, marginBottom: 8 }}>INDUSTRY PRESET</label>
            <select className="roi-input" value={industry} onChange={(e) => chooseIndustry(e.target.value)}>
              {ROI_INDUSTRIES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
            </select>

            {[
              ["Daily inquiries", inquiries, setInquiries, 1, 500],
              ["Delayed / missed (%)", delayedRate, setDelayedRate, 0, 95],
              ["Likely conversion if handled (%)", conversionRate, setConversionRate, 1, 100],
            ].map(([label, value, setter, min, max]) => (
              <div key={label} style={{ marginTop: 22 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
                  <span className="mono" style={{ color: G.muted, fontSize: 11 }}>{label}</span>
            ...
            <div style={{ marginTop: 22 }}>
              <label className="mono" style={{ display: "block", color: G.muted, fontSize: 10, marginBottom: 8 }}>AVERAGE VALUE OF A SUCCESSFUL CONVERSION</label>
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <input className="roi-input" type="number" min="1" value={avgValue} onChange={(e) => setAvgValue(Math.max(1, Number(e.target.value) || 1))} />
              </div>
            </div>
          </div>
          ...
          <div style={{ background: "#07070F", border: `1px solid ${G.green}33`, padding: "26px 22px" }}>
            ...
            {[
              ["Per day", estimate.daily],
              ["Per month", estimate.monthly],
              ["Per year", estimate.yearly],
            ].map(([label, value]) => (
            ...
            <p className="mono" style={{ color: G.muted, fontSize: 10, lineHeight: 1.7, marginTop: 18 }}>
              Formula: inquiries × delayed/missed rate × likely conversion rate × average conversion value. Presets are starting assumptions only.
            </p>
          </div>
        </div>
        ...
        <div style={{ marginTop: 18, background: G.card, border: `1px solid ${G.border}`, padding: "26px 22px" }}>
          ...
          <div className="grid-cards-3" style={{ gap: 12 }}>
            {ROI_PACKAGES.map((pkg) => (
              <button key={pkg.name} type="button" onClick={() => setSelectedPackage(pkg.name)} style={{ textAlign: "left", cursor: "pointer", background: selectedPackage === pkg.name ? `${G.green}0B` : "#080812", border: `1px solid ${selectedPackage === pkg.name ? G.green : G.border}`, padding: 18, color: G.white }}>
        ...
        <form onSubmit={handleSubmit} style={{ marginTop: 18, background: G.card, border: `1px solid ${G.border}`, padding: "26px 22px" }}>
          ...
          <div className="grid-cards-3" style={{ gap: 12 }}>
            <input className="roi-input" placeholder="Full name *" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} />
            <input className="roi-input" placeholder="WhatsApp with country code *" value={form.whatsapp} onChange={(e) => setForm((p) => ({ ...p, whatsapp: e.target.value }))} />
            <input className="roi-input" placeholder="Company / business" value={form.company} onChange={(e) => setForm((p) => ({ ...p, company: e.target.value }))} />
          </div>
          <input aria-hidden="true" tabIndex="-1" autoComplete="off" value={form.website} onChange={(e) => setForm((p) => ({ ...p, website: e.target.value }))} style={{ position: "absolute", left: "-9999px" }} />
          ...
          {error && <div className="mono" style={{ color: "#FF7777", marginTop: 14, fontSize: 11 }}>{error}</div>}
